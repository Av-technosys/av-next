...
function InputFiled({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        className="w-full rounded-md border border-gray-200 bg-transparent bg-white px-4 py-2 ring-blue-200 [-moz-appearance:textfield] focus:ring-2 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
...
function TextAreaField({ name, placeholder, value, onChange }: any) {
  return (
    <div className="flex flex-col gap-1">
      <p className="font-semibold">{name}</p>
      <textarea
        className="w-full rounded-md border border-gray-200 bg-transparent bg-white px-4 py-2 ring-blue-200 focus:ring-2"
        placeholder={placeholder}
        onChange={onChange}
        value={value}
        rows={4}
